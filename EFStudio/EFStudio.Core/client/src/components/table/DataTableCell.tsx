...
export function DataTableCell({ value, column }: DataTableCellProps) {
  ...
  if (column.type === "datetime") {
    ...
    return <span className="text-sm tabular-nums text-muted-foreground">{formatted}</span>;
  }

  if (column.type === "number") {
    return <span className="text-sm tabular-nums font-mono">{String(value)}</span>;
  }

  return (
    <span className="text-sm truncate block max-w-[240px]" title={String(value)}>
      {String(value)}
    </span>
  );
}
